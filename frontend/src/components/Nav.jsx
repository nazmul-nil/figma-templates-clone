...
const Nav = () => {
  ...
  return (
    <nav className='w-full bg-black' id='nav-wrapper'>
        <div className='max-w-[1170px] mx-auto'>
            <div className='flex items-center justify-between  py-[15px]'>
                <div className='lg:flex lg:items-center '>
                    
                    <div className='flex items-center gap-2 px-4 lg:px-0'>
                        {open ? <TfiClose className='text-white text-lg lg:hidden cursor-pointer' onClick={() => setOpen(!open)}/> : <FiMenu className='text-white text-2xl lg:hidden cursor-pointer' onClick={() => setOpen(!open)}/>}
                        <Link to={'/'} className=''>
                    ...
                    <div className={`flex flex-col lg:flex-row lg:items-center lg:ml-[150px] mt-4 lg:mt-0 px-4 py-4  absolute lg:static bg-white w-full lg:w-auto h-screen lg:h-auto lg:bg-inherit lg:z-auto z-10 ${open ? 'left-0' : 'left-[-900px]'} transition-all duration-500 ease-in-out`}>
                        <div className='relative'>
                            <div className='my-4 lg:my-0 flex items-center' onClick={() => setShowDropdown(!showDropdown)}>
                                <span className='text-[16px] text-black lg:text-[#ADADAD] hover:lg:text-white leading-4 font-semibold ml-4 mr-2  lg:my-0 cursor-pointer'>
                                    Our Products
                                </span>
                            ...
                            {/* dropdown menu */}
                            <div className={`lg:absolute lg:top-10 transition-all duration-300 ease-in-out ${showDropdown ? 'z-10 opacity-1' : 'opacity-0 hidden'}`}>
                                <Dropdown/>
                            </div>
                        </div>
